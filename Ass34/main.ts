...
function make_Album(artist: string, title: string, tracks?: number): Record<string, string | number> {
  ...
  const album: Record<string, string | number> = { artist, title };
  if (tracks) {
    album.tracks = tracks;
  }
  return album;
...
const array2 :string[] = ['jony','alex','Imran']
...
function make_great_new (copy:string[]):string[]{
...
const nmagic :string[] = ['jony','alex','Imran']
const newmagic :string [] = make_great_new(nmagic)
console.log(nmagic)
